...
class ContentModel:
    ...
    def __init__(self, movieData):
        print('Constructing Content Model ...')
        self.movieData = movieData
        self.movies = self.movieData.movies
        self.ratings = self.movieData.ratings.set_index('userId')
        self.ratingsTrain = self.movieData.ratingsTrain.set_index('userId')
        self.ratingsTest = self.movieData.ratingsTest.set_index('userId')
        self.lstMovieIds = self.ratings['movieId'].unique().tolist()
        ...
        self.TokenData = self.movieData.data[['movieId','original_title',\
                                              'overview','genres']]\
                             .drop_duplicates().set_index('movieId')
        self.TokenCorpus = self.TokenData['original_title'].fillna('') + " " + \
                           self.TokenData['overview'] + " " + \
                           self.TokenData['genres'].str.replace('|',' ')
        ...
        self.UserRatings_TFIDF = self.GenerateAllUsersProfile_TFIDF()
        self.AllUserTopTaste = self.GetUserTasteAllUsers()
    ...
    def GenerateUserProfile_TFIDF(self,UserId):
        ...
        UserRatings = self.ratingsTrain.loc[UserId]
    ...
    def GenerateAllUsersProfile_TFIDF(self):
        ...
        for UserId in self.ratingsTrain.index.unique():
    ...
    def GetUserTaste(self,UserId,n=10):
        UserTaste = pd.DataFrame(sorted(zip(self.TFIDF_Tokens, \
                                            ...
                                            self.UserRatings_TFIDF[UserId]\
                                                .flatten().tolist()),\
                                        key = lambda x: -x[1])[:n],\
                                 columns=['Token', 'RelevanceScore'])
        return UserTaste
    
    def GetUserTasteAllUsers(self):
        AllUserTaste = {}
        for UserId in self.ratingsTrain.index.unique():
            AllUserTaste[UserId] = self.GetUserTaste(UserId,25)
        return AllUserTaste
    ...
    def GetUserRatedMovies(self, UserId,ratings):
        RatedMovies = ratings.loc[UserId]['movieId']
        return RatedMovies
    ...
    def RecommendMovies(self, UserId,RecommendForTest = True):
        ...
        if RecommendForTest == True:
            ratings = self.movieData.ratingsTest.set_index('userId')
        else:
            ratings = self.movieData.ratingsTrain.set_index('userId')
        ...
        return UserRecommendations[['movieId','title','similarityScore']] 
